...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and pitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/pitcoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed pitcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './pitcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in pitcore-node services directory
