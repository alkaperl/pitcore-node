...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'pitcore-lib': '^' + bitcore.version,
    'pitcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/pitcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the pitcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the pitcore-node directory and configuration
